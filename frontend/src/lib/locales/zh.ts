export const zh = {
  ...
  chat: {
    ...
    runLimit: '运行上限',
    runLimitAutonomous: '自主模式',
    runLimitAutonomousHint: '由 LLM 判断任务是否完成；适合一次性操作任务。',
    runLimitSteps: '按步数',
    runLimitDuration: '按时长',
    runLimitUnlimited: '不限制',
    runLimitUnlimitedHint:
      '严格运行：任务将持续运行，直到手动停止或遇到错误/安全保护。',
    maxSteps: '最大执行步数',
    maxStepsHint: '严格按步数运行；不会因 LLM 判断完成而提前停止。',
    maxStepsRequired: '请填写最大执行步数',
    maxDuration: '最大持续工作时长（秒）',
    maxDurationHint: '严格按时长运行；不会因 LLM 判断完成而提前停止。',
    maxDurationRequired: '请填写最大持续工作时长',
    visionModelTab: '视觉模型',
    ...
    observationWindow: '动态观察窗口',
    observationWindowHint:
      '截图张数大于 1 时，后续任务每一步都会按该窗口采样多张截图；设为 1 则恢复单张截图。',
    observationWindowScreenshotCount: '截图张数',
    observationWindowIntervalSeconds: '截图间隔（秒）',
